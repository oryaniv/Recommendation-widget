...
(function(){
    ...
    window._oyRecommendations = window._oyRecommendations || {    
        ...
        styleUrls: [
            './style/widget.css',
            './style/widget-desktop.css',
            './style/widget-mobile.css'
        ],
        httpResourceParams:{
...
(function(){  
    ...
    (function(el,id,sel,css,js,hrp){
        ...
        const html = "<div class='list-container'>" +
                    "<div class='feed-header'>" +
                        "<div class='icon'></div>" +
                        "<h3 class='more-for'>More articles for you</h3>" +
                        "<ul id='article-list'>" +
                        "</ul>" +
                        "<div id='error-message' style='display:none'>There was a problem, sorry for the inconvenience</div>" + 
                        "<p id='loader-container' class='lt-ie9'><i class='loader loader--large'></i></p>" +
                        "<button id='anchor'>anchor</button>" +
                    "</div>";

        const styles = css.filter((c) => {
            return isMobile ? !c.includes('desktop') : !c.includes('mobile');
        }).map((c) => {
            return "<link rel='stylesheet' href='" + c + "'>";
        });
        const scripts =  js.map((s) => {
        ...
        const callback = function(){
            const button = frame.contentWindow.document.querySelector("#anchor");
            button.click();
        }
    ...
    })(document.getElementById('#' + _oyRecommendations.defaultElementId),
    _oyRecommendations.elementId,
    currentScript,
    _oyRecommendations.styleUrls,
    _oyRecommendations.scriptUrls,
    _oyRecommendations.httpResourceParams)
})();
